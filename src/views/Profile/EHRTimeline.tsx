  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
      <div className="bg-white/80 backdrop-blur-xl sticky top-0 z-10 border-b border-slate-100/50">
        ...
        >
          <span className="font-medium text-slate-800">完整健康档案 (EHR)</span>
        </NavBar>
      </div>

      <div className="p-6 flex-1">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-slate-900 tracking-tight mb-2">全病程管理轨迹</h1>
        ...
        <div className="relative pl-4">
          {/* Vertical Line */}
          <div className="absolute left-[27px] top-4 bottom-4 w-0.5 bg-slate-200/60 rounded-full" />
          ...
          <div className="space-y-8">
            {timelineData.map((item, index) => (
              ...
              >
                {/* Timeline Node */}
                <div className={`absolute left-0 top-1 w-6 h-6 rounded-full ${item.bgColor} border-2 ${item.borderColor} flex items-center justify-center z-10 shadow-sm`}>
                  <div className={`w-2 h-2 rounded-full ${item.color.replace('text-', 'bg-')}`} />
                </div>

                {/* Content Card */}
                <div className="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-slate-100/80">
                  ...
                  <div className="flex items-start gap-3 mb-2">
                    <div className={`p-2 rounded-xl ${item.bgColor} shrink-0 mt-0.5`}>
                      <item.icon className={`w-5 h-5 ${item.color}`} />
                    </div>
                    <div>
                      <h3 className="text-base font-semibold text-slate-900 leading-snug mb-1.5">{item.title}</h3>
