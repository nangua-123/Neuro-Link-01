    ...
    >
      <div className="space-y-6">
        ...
        {!summary && !isGenerating && (
          <div className="bg-indigo-50/50 border border-indigo-100 rounded-[16px] p-5 text-center space-y-3">
            <div className="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center mx-auto mb-2">
              <Sparkles className="w-6 h-6 text-indigo-500" />
            </div>
            <h4 className="text-[14px] font-bold text-indigo-900">AI 智能总结</h4>
            <p className="text-[12px] text-indigo-700/80 leading-relaxed">
              基于您近期的打卡记录、体征数据与量表结果，一键生成结构化的真实世界证据 (RWE) 摘要，方便医生快速了解您的病情变化。
            </p>
            <Button 
              block 
              className="mt-4 bg-indigo-600 text-white border-none rounded-[16px] h-12 font-bold text-[14px] shadow-[0_4px_12px_rgba(79,70,229,0.2)]"
        ...
        {isGenerating && (
          <div className="bg-white border border-slate-100 rounded-[16px] p-8 text-center space-y-4 shadow-sm">
            <Loader2 className="w-8 h-8 text-indigo-500 animate-spin mx-auto" />
            <p className="text-[13px] font-medium text-slate-600">正在分析多维健康数据...</p>
          </div>
        )}

        {summary && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-4">
            <div className="bg-white border border-slate-200 rounded-[20px] p-5 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 w-20 h-20 bg-indigo-50 rounded-full blur-2xl -mr-10 -mt-10" />
              <div className="flex items-center gap-2 mb-3 relative z-10">
                <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                <span className="text-[12px] font-bold text-slate-900">生成完毕</span>
              </div>
              <p className="text-[13px] text-slate-700 leading-relaxed relative z-10 text-justify">
                {summary}
              </p>
            </div>
            <Button 
