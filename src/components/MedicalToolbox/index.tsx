...
export function MedicalToolbox() {
  ...
  const tools = [
    ...
    { 
      ...
      onClick: () => showComingSoon('睡眠日志', '主人，更详细的睡眠日志功能即将上线。'),
    ...
    { 
      ...
      onClick: () => showComingSoon('用药记录', '主人，完整的用药记录与调整历史即将上线。'),
    ...
    { 
      ...
      onClick: () => showComingSoon('体检报告', '主人，历史体检报告与 AI 深度解读即将上线。'),
  ...
  return (
    <div className="space-y-3 mt-8">
      <div className="px-1 flex items-center justify-between">
        <h3 className="text-[15px] font-bold text-slate-900 tracking-tight">您的专属工具箱</h3>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {tools.map((tool, index) => (
          <motion.button
            ...
            className="bg-gradient-to-br from-white to-slate-50/80 rounded-[20px] p-3 shadow-[0_4px_16px_rgba(0,0,0,0.03)] border-none flex flex-col items-start text-left group hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] transition-all relative overflow-hidden"
          >
            <div className="flex items-center justify-between w-full mb-2">
              <div className={`w-8 h-8 rounded-[12px] ${tool.bg} flex items-center justify-center ${tool.color} shrink-0 group-hover:scale-110 transition-transform shadow-inner`}>
                <tool.icon className="w-4 h-4" />
              </div>
              {tool.status && (
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${getStatusStyles(tool.statusType)}`}>
                  {tool.status}
                </span>
              )}
            </div>
            ...
            <div className="w-full">
              ...
              <p className="text-[10px] text-slate-500 mt-0.5 font-medium truncate">{tool.desc}</p>
            </div>
          </motion.button>
        ))}
      </div>
