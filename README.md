...
- **布局规范**: 严格遵守移动端布局红线，使用 `h-[100dvh]` 配合 `flex-1 overflow-y-auto` 接管全局滚动，彻底杜绝了 `min-h-screen` 导致的滚动异常与底部白屏问题。

### Batch 27: 每日任务闭环逻辑重构 (Daily Task Closed-Loop Refactoring)
- **架构升级**: 在 `Task` 接口中引入 `actionRoute` 字段，实现任务驱动的动态路由分发。
- **脑力训练闭环**: 点击“每日脑力唤醒”不再是简单的 Toast 模拟，而是真实跳转至 `/feature/brain-training` 脑力极客大厅，用户选择具体训练项目（如舒尔特方格）并完成训练后，系统自动回调 `completeTask` 标记今日任务完成。
- **日记记录闭环**: 点击“记录异常行为”、“记录发作日记”等任务，将精准路由至对应的专病数据大屏（`/cdr`, `/diary`），用户在真实业务场景中提交表单后，底层 Store 自动拦截并核销对应的 Daily Task。
- **睡眠与音频闭环**: “规律作息打卡”与“暗室舒缓音频”任务同样接入真实路由，在 Feature 落地页完成交互后自动闭环。
- **UI/UX 规范**: 保持了 C 端消费级管家范式，任务跳转过程平滑自然，无缝衔接核心业务流。

### Batch 26: 用药打卡组件 UI/UX 深度优化 (Medication Check-in Polish)
- **Objective:** 优化用药打卡组件的样式，使其能正确显示药物名称、剂量和时间，并支持一键打卡。
- **Key Changes:**
  - **一键打卡快捷入口**: 在 `DailyTaskManager` 组件中，针对用药任务 (`MEDICATION`)，将右侧的箭头图标替换为专属的“打卡”按钮，支持在任务列表页直接一键完成服药记录。
  - **打卡确认弹窗重构**: 深度优化了 `ManagerView` 中的用药确认 Modal。
